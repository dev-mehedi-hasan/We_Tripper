...
class Home extends CI_Controller {
    ...
    function contact_us($listing_type = "") {
        ...
        elseif ($listing_type == 'flights') {
            ...
            $data['flights_cabin'] = $this->input->post('flights_cabin');
            $data['adult_guests_for_booking'] = sanitizer($this->input->post('adult_guests_for_booking'));
            $data['child_guests_for_booking'] = sanitizer($this->input->post('child_guests_for_booking'));
            $data['to'] = $listing_details['email'];
            if ($data['booking_date'] != "" && $data['flights_cabin'] != "" && $data['departure_time'] != "") {
                $this->frontend_model->flights_booking();
                $this->email_model->flights_booking_mail($data);
            }
            else {
                $this->session->set_flashdata('error_message', get_phrase('fill_all_fields_first'));
                //redirect to routs file
        ...
        elseif ($listing_type == 'ship_ticket') {
            $date= $this->input->post('departure_date');

            $data['booking_date'] = $date;
            $departure_time= $this->input->post('departure_time');
            $data['departure_time'] = $departure_time;
            $data['ship_cabin'] = $this->input->post('ship_cabin');
            $data['adult_guests_for_booking'] = sanitizer($this->input->post('adult_guests_for_booking'));
            $data['child_guests_for_booking'] = sanitizer($this->input->post('child_guests_for_booking'));
            $data['to'] = $listing_details['email'];
            if ($data['booking_date'] != "" && $data['ship_cabin'] != "" && $data['departure_time'] != "") {
                $this->frontend_model->ship_booking();
                $this->email_model->ship_booking_mail($data);
            }
            else {
                $this->session->set_flashdata('error_message', get_phrase('fill_all_fields_first'));
                //redirect to routs file
        ...
        elseif ($listing_type == 'tour_package') {
            $data['tour_package_service_type'] = $this->input->post('tour_package_service_type');
            $data['adult_guests_for_booking'] = sanitizer($this->input->post('adult_guests_for_booking'));
            $data['child_guests_for_booking'] = sanitizer($this->input->post('child_guests_for_booking'));
            $data['to'] = $listing_details['email'];
            if ($data['tour_package_service_type'] != "") {
                $this->frontend_model->tour_package_booking();
                $this->email_model->tour_package_booking_mail($data);
            }
            else {
                $this->session->set_flashdata('error_message', get_phrase('fill_all_fields_first'));
                //redirect to routs file
        ...
        elseif ($listing_type == 'car_rental') {
            $dates= explode('>', $this->input->post('dates'));
            $data['book_from'] = $dates[0];
            $data['book_to'] = $dates[1];

            $data['car_rental_pick_up_city'] = $this->input->post('car_rental_pick_up_city');
            $data['car_rental_pick_up_point'] = $this->input->post('car_rental_pick_up_point');
            $data['to'] = $listing_details['email'];
            if ( $data['book_from'] != "" && $data['book_to'] != "" && $data['car_rental_pick_up_city'] != "") {
                $this->frontend_model->car_rental_booking();
                $this->email_model->car_rental_booking_mail($data);
            }
            else {
                $this->session->set_flashdata('error_message', get_phrase('fill_all_fields_first'));
                //redirect to routs file
        ...
        elseif ($listing_type == 'visa') {
            $data['visa_booking_country'] = $this->input->post('visa_booking_country');
            $data['visa_booking_type'] = $this->input->post('visa_booking_type');
            $data['visa_booking_message'] = $this->input->post('visa_booking_message');
            $data['to'] = $listing_details['email'];
            if ( $data['visa_booking_country'] != "" && $data['visa_booking_type'] != "") {
                $this->frontend_model->visa_booking();
                $this->email_model->visa_booking_mail($data);
            }
            else {
                $this->session->set_flashdata('error_message', get_phrase('fill_all_fields_first'));
                //redirect to routs file
