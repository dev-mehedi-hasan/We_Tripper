...
function add_listing() {
  ...
  $data['regular_price'] = sanitizer($this->input->post('regular_price'));
  $data['sale_price'] = sanitizer($this->input->post('sale_price'));
...
function add_listing_type_wise_details($listing_type = "", $listing_id = "") {
  ...
  // if ($listing_type == 'flights') {
  //   $cabin_name_array = sanitizer($this->input->post('cabin_name'));
  //   array_pop($cabin_name_array);
  //   $cabin_description_array = sanitizer($this->input->post('cabin_description'));
  //   array_pop($cabin_description_array);
  //   $cabin_price_array = sanitizer($this->input->post('cabin_price'));
  //   array_pop($cabin_price_array);
  //   $flights_cabin_amenities_array = sanitizer($this->input->post('flights_cabin_amenities'));
  //   array_pop($flights_cabin_amenities_array);
  //   foreach ($_FILES['cabin_image']['tmp_name'] as $cabin_image) {
  //     if ($cabin_image != "") {
  //       $random_identifier = md5(rand(10000000, 20000000)).'.jpg';
  //       move_uploaded_file($cabin_image, 'uploads/flights_cabin_images/'.$random_identifier);
  //       array_push($listing_photos, $random_identifier);
  //     }
  //   }

  //   foreach ($cabin_name_array as $key => $cabin_name) {
  //     $flights_cabin_specification_data['name']        = sanitizer($cabin_name);
  //     $flights_cabin_specification_data['description'] = sanitizer($cabin_description_array[$key]);
  //     $flights_cabin_specification_data['price']       = sanitizer($cabin_price_array[$key]);
  //     $flights_cabin_specification_data['amenities']   = sanitizer($flights_cabin_amenities_array[$key]);
  //     $flights_cabin_specification_data['photo']       = sanitizer($listing_photos[$key]);
  //     $flights_cabin_specification_data['listing_id']  = $listing_id;
  //     $this->db->insert('flights_cabin_specification',$flights_cabin_specification_data);
  //   }
  // }
}
...
function update_listing_type_wise_details($listing_type = "", $listing_id = "") {
  ...
  //   $flights_cabin_amenities_array = sanitizer($this->input->post('flights_cabin_amenities'));
  //   array_pop($flights_cabin_amenities_array);
  //   $flights_cabin_ids_array = sanitizer($this->input->post('flights_cabin_id'));
  //   array_pop($flights_cabin_ids_array);

  //   // Image Uploading functions starts here
  //   $old_flights_cabin_images = sanitizer($this->input->post('old_flights_cabin_images'));
  //   array_pop($old_flights_cabin_images);
  //   // Image Uploading functions ends here

  //   foreach ($_FILES['cabin_image']['tmp_name'] as $key => $cabin_image) {
  //     if ($cabin_image != "") {
  //       $random_identifier = md5(rand(10000000, 20000000)).'.jpg';
  //       move_uploaded_file($cabin_image, 'uploads/flights_cabin_images/'.$random_identifier);
  //       array_push($listing_photos, $random_identifier);
  //     }else {
  //       array_push($listing_photos, $old_flights_cabin_images[$key]);
  //     }
  //   }

  //   foreach ($flights_cabin_ids_array as $key => $flights_cabin_id) {
  //     $flights_cabin_specification_data['name']        = sanitizer($cabin_name_array[$key]);
  //     $flights_cabin_specification_data['description'] = sanitizer($cabin_description_array[$key]);
  //     $flights_cabin_specification_data['price']       = sanitizer($cabin_price_array[$key]);
  //     $flights_cabin_specification_data['amenities']   = sanitizer($flights_cabin_amenities_array[$key]);
  //     $flights_cabin_specification_data['photo']       = sanitizer($listing_photos[$key]);
  //     $flights_cabin_specification_data['listing_id']  = sanitizer($listing_id);
  //     if ($flights_cabin_id > 0) {
  //       $this->db->where('id', $flights_cabin_id);
  //       $this->db->update('flights_cabin_specification', $flights_cabin_specification_data);
  //     }else {
  //       $this->db->insert('flights_cabin_specification',$flights_cabin_specification_data);
  //     }
  //   }
