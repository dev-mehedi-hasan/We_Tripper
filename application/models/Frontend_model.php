...
class Frontend_model extends CI_Model {
    ...
    function get_top_three_listings() {
        $this->db->where('listing_type','tour_package');
        $this->db->order_by('id','DESC');
        $this->db->limit(3);  
        return $this->db->get('listing');
    }
    function get_featured_two_hotels() {
        $this->db->where('listing_type','hotel');
    ...
    function flights_booking(){
        $data['user_id']                  = $this->input->post('user_id');
        $data['requester_id']                  = $this->input->post('requester_id');
        $data['listing_id']               = $this->input->post('listing_id');
        $data['listing_type']               = $this->input->post('listing_type');

        $booking_date= $this->input->post('departure_date');
        ...
        $additional_data['adult_guests']  = $this->input->post('adult_guests_for_booking');
        $additional_data['child_guests']  = $this->input->post('child_guests_for_booking');
        $additional_data['flights_cabin']          = $this->input->post('flights_cabin');
        $additional_data['departure_time']          = $this->input->post('departure_time');
        $data['additional_information']   = json_encode($additional_data);
    ...
    function ship_booking(){
        ...
        $additional_data['adult_guests']  = $this->input->post('adult_guests_for_booking');
        $additional_data['child_guests']  = $this->input->post('child_guests_for_booking');
        $additional_data['ship_cabin']          = $this->input->post('ship_cabin');
        $additional_data['departure_time']          = $this->input->post('departure_time');
        $data['additional_information']   = json_encode($additional_data);
    ...
    function tour_package_booking(){
        ...
        $additional_data['adult_guests']  = $this->input->post('adult_guests_for_booking');
        $additional_data['child_guests']  = $this->input->post('child_guests_for_booking');
        $additional_data['tour_package_service_type']          = $this->input->post('tour_package_service_type');
        $data['additional_information']   = json_encode($additional_data);
    ...
    function car_rental_booking(){
        ...
        $data['booking_date']             = strtotime($fromdate->date).' - '.strtotime($todate->date);
        $additional_data['car_rental_pick_up_city']  = $this->input->post('car_rental_pick_up_city');
        $additional_data['car_rental_pick_up_point']  = $this->input->post('car_rental_pick_up_point');
        $data['additional_information']   = json_encode($additional_data);
    ...
    function visa_booking(){
        ...
        $additional_data['visa_booking_country']  = $this->input->post('visa_booking_country');
        $additional_data['visa_booking_type']  = $this->input->post('visa_booking_type');
        $additional_data['visa_booking_message']  = $this->input->post('visa_booking_message');
        $data['additional_information']   = json_encode($additional_data);
    ...
    function get_top_three_package_listings() {
        $package_listing_ids = array();
        $package_listing_id_with_rating = array();
        $package_listings = $this->get_top_three_listings()->result_array();
        foreach ($package_listings as $package_listing) {
          if(!has_package($package_listing['user_id']) > 0){
            continue;
          }
          $package_listing_id_with_rating[$package_listing['id']] = $this->get_listing_wise_rating($package_listing['id']);
        }
        arsort($package_listing_id_with_rating);
        foreach ($package_listing_id_with_rating as $key => $value) {
            if (count($package_listing_ids) <= 3) {
                array_push($package_listing_ids, $key);
            }
        }
        if (count($package_listing_ids) > 0) {
            $this->db->where_in('id', $package_listing_ids);
            $this->db->where('status', 'active');
            return  $this->db->get('listing')->result_array();
        }else {
            return array();
    ...
    function get_featured_two_listing_hotels() {
        $hotel_listing_ids = array();
        $hotel_listing_id_with_rating = array();
        $hotel_listings = $this->get_featured_two_hotels()->result_array();
        foreach ($hotel_listings as $hotel_listing) {
          if(!has_package($hotel_listing['user_id']) > 0){
            continue;
          }
          $hotel_listing_id_with_rating[$hotel_listing['id']] = $this->get_listing_wise_rating($hotel_listing['id']);
        }
        arsort($hotel_listing_id_with_rating);
        foreach ($hotel_listing_id_with_rating as $key => $value) {
            if (count($hotel_listing_ids) <= 2) {
                array_push($hotel_listing_ids, $key);
            }
        }
        if (count($hotel_listing_ids) > 0) {
            $this->db->where_in('id', $hotel_listing_ids);
            $this->db->where('status', 'active');
            return  $this->db->get('listing')->result_array();
        }else {
            return array();
